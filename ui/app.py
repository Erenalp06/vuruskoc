"""
VurusKoc Gradio UI (ic test araci) — tenis vurus analizi.
...
def analyze_video(video_path, playing_hand='right', lang='tr', stroke='auto'):
    """Full swing analysis pipeline on uploaded video."""
    coaching_engine.set_lang(lang)
    msg_no_video = "Video yüklenmedi." if lang == 'tr' else "No video uploaded."
    msg_unreadable = "Video okunamadı." if lang == 'tr' else "Could not read video."
    if video_path is None:
        return None, msg_no_video, "{}"

    frames = read_video(video_path, max_dim=1280, max_frames=900)
    if not frames:
    ...
    # Find contact frame (ball-free heuristic: hand out front, near shoulder
    # height, moving fast — not just peak wrist speed).
    side = playing_hand
    hint = stroke if stroke != 'auto' else 'auto'
    contact_idx = swing_classifier.find_contact_frame(
        keypoints_seq, velocities, phases, stroke_hint=hint, side=side)
    if contact_idx is None:
        contact_idx = int(np.argmax(velocities))

    contact_angles = pose_engine.get_joint_angles(keypoints_seq[contact_idx], side=side)

    # Get BEST loading angles from the loading phase (peak coil / deepest knee
    # bend / max racket lag), falling back to the tail of preparation.
    loading_angles = swing_classifier.best_loading_angles(
        pose_engine, keypoints_seq, phases, side=side)

    # Stroke: honour the explicit choice, else auto-detect.
    if stroke != 'auto':
        stroke_type = stroke
    else:
        stroke_type = swing_classifier.classify_stroke(
            keypoints_seq[contact_idx], keypoints_seq, phases, side=side)
    ...
    if warnings:
        report += "\n\n" + coaching_engine.L['injury_header'] + "\n" + \
...
def _bar_color(score):
    return '#34d399' if score >= 80 else '#fbbf24' if score >= 50 else '#f87171'


def _comparison_html(pro_name, pro_stroke, stroke_label, rows, scores,
                     corrections, lang='tr'):
    """rows: list of (metric_label, you, pro, diff, score). corrections: list of
    dicts {name, value, lo, hi, score, drill, pro, diff}."""
    tr = lang == 'tr'
    overall = scores.get('overall', 0)
    ring = _bar_color(overall)
    verdict = ''
    for th in (85, 70, 50, 0):
        if overall >= th:
            verdict = coaching_engine.L['verdict'][th]
            break

    t_you, t_pro, t_diff = (('Sen', 'Pro', 'Fark') if tr else ('You', 'Pro', 'Δ'))
    hdr_metrics = 'Kontak açıları — sen vs pro' if tr else 'Contact angles — you vs pro'
    hdr_fix = 'Öncelikli düzeltmeler' if tr else 'Priority fixes'
    lbl_stroke = 'Vuruş' if tr else 'Stroke'
    lbl_proclip = 'Pro klip' if tr else 'Pro clip'
    lbl_drill = 'Alıştırma' if tr else 'Drill'
    lbl_ideal = 'ideal'

    metric_rows = ""
    for name, you, pro, diff, sc in rows:
        c = _bar_color(sc)
        # marker offset: clamp |diff| to 90° -> +/-46% of the track
        off = max(-1.0, min(1.0, diff / 90.0)) * 46
        metric_rows += f"""
        <div style="display:grid;grid-template-columns:130px 62px 62px 1fr;gap:10px;
             align-items:center;padding:7px 0;border-top:1px solid #ffffff14;font-size:13px">
          <span style="color:#cbd5e1">{name}</span>
          <span style="color:#f8fafc;font-weight:600;text-align:right">{you:.0f}°</span>
          <span style="color:#94a3b8;text-align:right">{pro:.0f}°</span>
          <span style="position:relative;height:14px">
            <span style="position:absolute;left:50%;top:0;width:2px;height:14px;background:#64748b"></span>
            <span style="position:absolute;left:calc(50% + {off:.0f}%);top:1px;
                 transform:translateX(-50%);width:12px;height:12px;border-radius:50%;
                 background:{c};box-shadow:0 0 0 3px {c}33"></span>
          </span>
        </div>"""

    fix_cards = ""
    for i, c in enumerate(corrections[:2], 1):
        col = _bar_color(c['score'])
        pro_bit = ""
        if c.get('pro') is not None:
            pro_bit = (f"<div style='color:#94a3b8;font-size:12px;margin-top:3px'>"
                       f"{pro_name}: {c['pro']:.0f}° · "
                       f"{'sapma' if tr else 'off by'} {c['diff']:+.0f}°</div>")
        fix_cards += f"""
        <div style="background:#0b1220;border:1px solid #ffffff14;border-left:3px solid {col};
             border-radius:8px;padding:10px 12px;margin-top:8px">
          <div style="display:flex;justify-content:space-between;align-items:baseline">
            <b style="color:#f1f5f9;font-size:13px">{i}. {c['name']}</b>
            <span style="color:{col};font-weight:700;font-size:13px">{c['score']:.0f}/100</span>
          </div>
          <div style="color:#cbd5e1;font-size:12px;margin-top:2px">
            {c['value']:.0f}° · {lbl_ideal} {c['lo']}–{c['hi']}°</div>
          {pro_bit}
          <div style="color:#e2e8f0;font-size:12px;margin-top:6px">
            <span style="color:{col}">▸ {lbl_drill}:</span> {c['drill']}</div>
        </div>"""

    return f"""
    <div style="font-family:ui-sans-serif,system-ui,sans-serif;background:#111827;
         border:1px solid #ffffff1f;border-radius:12px;padding:16px 18px;color:#e5e7eb">
      <div style="display:flex;align-items:center;gap:16px">
        <div style="flex:none;width:66px;height:66px;border-radius:50%;
             background:conic-gradient({ring} {overall*3.6:.0f}deg,#ffffff14 0);
             display:flex;align-items:center;justify-content:center">
          <div style="width:52px;height:52px;border-radius:50%;background:#111827;
               display:flex;flex-direction:column;align-items:center;justify-content:center">
            <b style="font-size:17px;color:#f8fafc">{overall:.0f}</b>
            <span style="font-size:8px;color:#94a3b8;letter-spacing:.5px">/100</span>
          </div>
        </div>
        <div>
          <div style="font-size:12px;color:#94a3b8">SwingScore</div>
          <div style="font-size:14px;color:#f1f5f9;margin-top:2px">{verdict}</div>
          <div style="font-size:11px;color:#64748b;margin-top:4px">
            {t_you} vs <b style="color:#cbd5e1">{pro_name}</b> ·
            {lbl_stroke}: {stroke_label} · {lbl_proclip}: {pro_stroke}</div>
        </div>
      </div>

      <div style="margin-top:14px;font-size:12px;color:#94a3b8;letter-spacing:.4px">
        {hdr_metrics.upper()}</div>
      <div style="display:grid;grid-template-columns:130px 62px 62px 1fr;gap:10px;
           font-size:10px;color:#64748b;padding-bottom:2px">
        <span></span><span style="text-align:right">{t_you}</span>
        <span style="text-align:right">{t_pro}</span><span>{t_diff}</span>
      </div>
      {metric_rows}

      <div style="margin-top:16px;font-size:12px;color:#94a3b8;letter-spacing:.4px">
        {hdr_fix.upper()}</div>
      {fix_cards or '<div style="color:#64748b;font-size:12px;margin-top:6px">—</div>'}
    </div>"""


def compare_to_pro(video_path, pro_selection, playing_hand='right',
                   lang='tr', stroke='auto'):
    """Ghost-overlay the user's swing against a pro's skeleton, time-aligned at
    contact, and return (overlay_video_path, comparison_report)."""
    from modules.pro_overlay import GHOSTABLE_PROS, load_pro_sequence, render_overlay
    coaching_engine.set_lang(lang)
    tr = lang == 'tr'
    if video_path is None:
        return None, "Video yüklenmedi." if tr else "No video uploaded."

    frames = read_video(video_path, max_dim=1280, max_frames=900)
    if not frames:
        return None, "Video okunamadı." if tr else "Could not read video."
    ...
    contact_idx = swing_classifier.find_contact_frame(
        keypoints_seq, velocities, phases, stroke_hint=hint, side=side)
    if contact_idx is None or keypoints_seq[contact_idx] is None:
        return None, ("Vuruş anında poz bulunamadı." if tr
                      else "No pose detected at contact.")
    ...
    pro_key = GHOSTABLE_PROS.get(pro_selection, 'djokovic_forehand')
    pro = load_pro_sequence(pro_key)
    pro_json = json.load(open(os.path.join(PRO_DATA_DIR, f"{pro_key}.json")))
    pro_angles = pro_json.get('contact_angles', {})
    pro_name = pro_json.get('player', pro_selection)

    from core.coaching import IDEAL_RANGES, WEIGHTS
    merged = dict(angles or {})
    if loading_angles:
        merged.update(loading_angles)

    pro_comp = coaching_engine.compare_to_pro(merged, pro_key.split('_')[0], stroke_type)
    scores = coaching_engine.score_swing(angles, stroke_type, loading_angles=loading_angles) or {}
    ...
    for m in ['elbow_angle', 'hip_rotation', 'shoulder_angle', 'knee_angle', 'racket_lag']:
        if m not in merged or m not in pro_angles:
            continue
        sc = scores.get(m, 50.0)
        name = coaching_engine.L['metric_names'].get(m, m)
        rows.append((name, merged[m], pro_angles[m], merged[m] - pro_angles[m], sc))
        if sc < 90:
            lo, hi = ideals.get(m, (0, 0))
            ranked.append({
                'name': name, 'value': merged[m], 'lo': lo, 'hi': hi, 'score': sc,
                'drill': coaching_engine._get_drill(m, merged[m], (lo, hi)),
                'pro': pro_angles.get(m), 'diff': merged[m] - pro_angles[m],
                'impact': WEIGHTS.get(m, 0.1) * (100 - sc),
            })
    ranked.sort(key=lambda x: -x['impact'])

    html = _comparison_html(pro_name, pro_json.get('stroke'),
                            coaching_engine.stroke_name(stroke_type),
                            rows, scores, ranked, lang=lang)

    out_path = os.path.join(tempfile.gettempdir(), f"ghost_{os.getpid()}.mp4")
    video = render_overlay(frames, keypoints_seq, contact_idx, pro, side=side,
                           out_path=out_path, phases=phases)
    if video is None:
        html = ("<div style='color:#f87171;font:13px system-ui;padding:8px'>"
                + ("Overlay çizilemedi — poz hizalaması başarısız." if tr
                   else "Could not render overlay — alignment failed.")
                + "</div>" + html)
    return video, html
...
def build_app():
    with gr.Blocks(title="VurusKoc") as app:
        gr.Markdown(
            "# VuruşKoç · iç test arayüzü\n"
            "### Video yükle · vuruşu kes · analiz et\n"
            "---"
        )
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Upload & Analyze"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        lang_select = gr.Radio(
                            choices=[('Türkçe', 'tr'), ('English', 'en')], value='tr',
                            label="Rapor dili / Report language"
                        )
                        stroke_select = gr.Radio(
                            choices=[('Otomatik', 'auto'), ('Forehand', 'forehand'),
                                     ('Backhand', 'backhand'), ('Servis', 'serve')],
                            value='auto', label="Vuruş tipi / Stroke"
                        )
                ...
                analyze_btn.click(
                    fn=analyze_video,
                    inputs=[upload_video, hand_select, lang_select, stroke_select],
            ...
            with gr.TabItem("Pro Compare"):
                from modules.pro_overlay import GHOSTABLE_PROS
                with gr.Row():
                    with gr.Column(scale=1):
                        compare_video = gr.Video(label="Upload your swing video")
                        pro_select = gr.Dropdown(
                            choices=list(GHOSTABLE_PROS.keys()),
                        ...
                        lang_compare = gr.Radio(
                            choices=[('Türkçe', 'tr'), ('English', 'en')], value='tr',
                            label="Rapor dili / Report language"
                        )
                        stroke_compare = gr.Radio(
                            choices=[('Otomatik', 'auto'), ('Forehand', 'forehand'),
                                     ('Backhand', 'backhand'), ('Servis', 'serve')],
                            value='auto', label="Vuruş tipi / Stroke"
                        )
                    ...
                    with gr.Column(scale=1):
                        compare_overlay = gr.Video(label="Ghost Overlay (sen + pro)")
                compare_text = gr.HTML(label="Comparison Report")

                compare_btn.click(
                    fn=compare_to_pro,
                    inputs=[compare_video, pro_select, hand_compare, lang_compare, stroke_compare],
                    outputs=[compare_overlay, compare_text],
                )
