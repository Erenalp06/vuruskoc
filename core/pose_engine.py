...
class PoseEngine:
    ...
    def get_joint_angles(self, keypoints, side='right'):
        ...
        # Image coords: smaller y = higher. When the wrist is near the top of
        # frame wrist_y -> 0 and a raw ratio explodes (or flips sign), so clamp
        # the denominator and cap the ratio to a sane band.
        hip_y = keypoints[f'{s}_hip'][1]
        wrist_y = max(keypoints[f'{s}_wrist'][1], 1.0)
        angles['contact_height_ratio'] = float(np.clip(hip_y / wrist_y, 0.0, 6.0))
